...
export function getActiveCategories(database = getDatabase()) {
  return database
    .prepare(
      `
...
export function getActiveMenuItems(database = getDatabase(), { categorySlug } = {}) {
...
export function getMenuCatalog(database = getDatabase()) {
  const categories = getActiveCategories(database);
  const items = getActiveMenuItems(database);
...
export function getMenuItemById(menuItemId, database = getDatabase()) {
  const row = database
    .prepare(
      `
        ${baseMenuItemSelect}
...
export function getMenuItemCustomization(menuItemId, database = getDatabase()) {
  const item = getMenuItemById(menuItemId, database);
