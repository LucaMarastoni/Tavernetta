...
const CATEGORY_META_BY_KEY = {
  ...
  vini: { name: 'Vini', slug: 'vini' },
  'i vini': { name: 'Vini', slug: 'vini' },
  'vini bianchi': { name: 'Vini Bianchi', slug: 'vini-bianchi' },
  bianchi: { name: 'Vini Bianchi', slug: 'vini-bianchi' },
  'vini rose': { name: 'Vini Rose', slug: 'vini-rose' },
  rose: { name: 'Vini Rose', slug: 'vini-rose' },
  'vini rossi': { name: 'Vini Rossi', slug: 'vini-rossi' },
  rossi: { name: 'Vini Rossi', slug: 'vini-rossi' },
};
...
function resolveSupabaseImageUrl(client, imagePath) {
  ...
  if (cleanedPath.startsWith('/')) {
    return withBase(cleanedPath);
  }

  const bucket = getBrowserSupabaseStorageBucket();
...
function createSupabaseError(error, fallbackMessage) {
  ...
  nextError.code = error?.code || 'SUPABASE_QUERY_FAILED';
  nextError.details = error?.message || null;
  return nextError;
}
...
async function loadSupabaseRelations(client, menuItemIds) {
  ...
  const extras = extraIds.length
    ? await runSupabaseQuery(
        client.from('extra_ingredients').select('id, ingredient_id, extra_price, sort_order, active').in('id', extraIds),
        { optional: true, fallbackValue: [] },
      )
    : [];
...
async function getSupabaseActiveCategories(client, categorySlug) {
...
async function getSupabaseItemRows(client, categoryIds) {
...
async function getSupabaseCatalogData(client, { categorySlug } = {}) {
  const categories = await getSupabaseActiveCategories(client, categorySlug);
  ...
  const itemRows = await getSupabaseItemRows(client, categories.map((category) => category.id));
...
async function getSupabaseMenuItemRow(client, menuItemId) {
  ...
  const categories = await getSupabaseActiveCategories(client);
...
export function canUseSupabaseMenuSource() {
  return hasBrowserSupabaseConfig();
}

export async function fetchMenuCatalogFromSupabase() {
  const client = getConfiguredClient();
  const { categories, items } = await getSupabaseCatalogData(client);
...
export async function fetchMenuItemCustomizationFromSupabase(menuItemId) {
  const client = getConfiguredClient();
